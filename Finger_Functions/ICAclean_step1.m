function [ datain,bad_data ] = ICAclean_step1( rawdata,N,filtOrder,in )
% datain=ICAclean_step1(rawdata,N,in)
% Epochize raw continuous data into length N by indices in & write data
% input file with egi-256 head model


if nargin < 3 || isempty(filtOrder);
    filtOrder=2;                            % default filter order
end

% Filter the data (nth order, 50 Hz Butterworth filter)
rawdata=butterfilt(rawdata(1:256,:),filtOrder,50,1000);
...
datain.data=data(:,EGIHC256RED.ChansUsed,1:(size(data,3)));
end
